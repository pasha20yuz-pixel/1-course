#include "memstat.h"
#include "ast.c"  // чтобы видеть определения AstNode и функций
#include <ctype.h>
#include <string.h>
...
// ---------- вспомогательные структуры для парсеров ----------
typedef struct {
    AstNode** arr;
    int top;
    int cap;
...
// ---------- инфиксный парсер (сортировочная станция) ----------
typedef struct {
    char op;
    int prec;
    int right_assoc;
    ...
    *error = 0;
    ...
    while ((tok = lexer_next()).type != TOKEN_END && !*error) {
        if (tok.type == TOKEN_NUMBER) {
            ...
            while (operators->top >= 0 && (char)(intptr_t)top_node(operators) != '(') {
                char top_op = (char)(intptr_t)top_node(operators);
