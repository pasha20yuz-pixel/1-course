    ...
    switch (node->type) {
        ...
        case NODE_BINARY_OP: {
            ...
            switch (node->binary.op) {
                ...
                case '-': return left - right;
                case '*': return left * right;
                case '/': if (right == 0) { *error = 1; return 0; } return left / right;
                case '%': if (right == 0) { *error = 1; return 0; } return left % right;
                case '^': if (right < 0) { *error = 1; return 0; } return pow_ll(left, right);
                case '#': return gcd(left, right);
